...
% Author: C Pagniello
% Original: Oct 2017
% Last Edit: Jan 2018 (to v2) by J Butler and C Pagniello
...
MAIN.verno=' Image Processing v.1';
MAIN.figwidth = 0.80;
MAIN.figheight = 0.75;
...
if ~isfield(PARAMS,'ifile')
    ...
    if PARAMS.ifile ~= 0
        cd(PARAMS.idir);
        PARAMS.files = dir('*.jpg');
    else
        close all
        uiwait(msgbox('User canceled file selection - program terminates','Image Processing'));
        return
    end
...
PARAMS.hitime=uicontrol(MAIN.hf1,'style','edit','un','n','pos',[.915 .742 .085 .032], ...
    'str',[num2str(PARAMS.ifile(8:9)) ':' num2str(PARAMS.ifile(10:11)) ':' num2str(PARAMS.ifile(12:13))],'foreg',[0 0 1],'backg',[1 1 1]);
PARAMS.hiname=uicontrol(MAIN.hf1,'style','edit','un','n','pos',[.915 .710 .085 .032], ...
